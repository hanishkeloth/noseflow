...
export function createScene(T, host, input, settings) {
  ...
  let frame=0,last=performance.now(),time=0,disposed=false;
  ...
  return {renderer,dispose(){disposed=true;cancelAnimationFrame(frame);observer.disconnect();geometry.dispose();material.dispose();for(const mesh of ribbons.children){mesh.geometry.dispose();mesh.material.dispose();}renderer.dispose();renderer.domElement.remove();}};
}
