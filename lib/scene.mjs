...
export function createScene(T, host, input, settings, orbit) {
  ...
  let last=null,time=0,slowFrames=0;
  const smooth={x:0,y:0,distance:1};
  ...
  function animate(now){const dt=last===null?0:Math.min((now-last)/1000,.05);last=now;
    const s=settings.current;
    smooth.x=damp(smooth.x,input.current.x,dt);smooth.y=damp(smooth.y,input.current.y,dt);smooth.distance=damp(smooth.distance,s.distance??1,dt);
    const pose=orbit?.tick(now,smooth);
    if(!s.paused&&!pose)time+=dt*s.speed;
    ...
    // A take records the camera alone against a deterministic, frozen scene.
    tilt.update(s,pose?{x:0,y:0}:input.current,pose?0:time,dt,Boolean(pose));
    if(pose){
      const a=pose.azimuth*Math.PI/180,e=pose.elevation*Math.PI/180,r=7*pose.distance;
      camera.position.set(Math.sin(a)*Math.cos(e)*r,Math.sin(e)*r,Math.cos(a)*Math.cos(e)*r);
      uniforms.time.value=ribbonUniforms.time.value=0;
      ribbonUniforms.steer.value.set(0,0);
      points.rotation.y=points.rotation.z=0;
    }else{
      camera.position.x=damp(camera.position.x,input.current.x*s.sensitivity*.5,dt);
      camera.position.y=damp(camera.position.y,input.current.y*s.sensitivity*.35,dt);
      camera.position.z=damp(camera.position.z,7*(s.distance??1),dt);
      points.rotation.y=damp(points.rotation.y,input.current.x*.4,dt);points.rotation.z=time*.06;
    }
    camera.lookAt(0,0,0);
    renderer.render(scene,camera);
  }
  const loop=createFrameLoop(animate,{element:host,onSuspend(){last=null;orbit?.stop();}});loop.start();
  return {renderer,dispose(){loop.dispose();observer.disconnect();tilt.dispose();geometry.dispose();material.dispose();for(const mesh of ribbons.children){mesh.geometry.dispose();mesh.material.dispose();}renderer.dispose();renderer.domElement.remove();}};
}
