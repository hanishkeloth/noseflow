...
export function createTiltStudio(T, scene) {
  ...
  return {
    update(s,input,time,dt,locked=false){
      ...
      group.rotation.x=locked?-.08:damp(group.rotation.x,-.08-input.y*.18,dt);
      group.rotation.y=locked?-.25:damp(group.rotation.y,-.25+input.x*.32,dt);
